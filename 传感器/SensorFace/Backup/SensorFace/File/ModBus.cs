...
namespace SensorFace
{
    ...
    class ModBusClass
    {
        ...
        public const int TargetAddr = 0xfe;
        ...
        public static int ModBus_Expend(byte[] Buf, int Length, ref MB ModBusMsg)
        {
            ...
            if (ModBusMsg.Start != Buf[i++]) { ModBusMsg.ErrorFlag = ModBus_FlagError; return 0; }
            if (ModBusMsg.WorkMode == (byte)MasterSlaveMode.WorkMode_Master)//当协议运行为主机模式
            {
                if (ModBusMsg.MasterAddr != Buf[i++]) { ModBusMsg.ErrorFlag = ModBus_AddrError; return 0; }
                if (ModBusMsg.SlaveAddr != Buf[i++]) ;
            }
            else if (ModBusMsg.WorkMode == (byte)MasterSlaveMode.WorkMode_Slave)//当协议运行在从机模式
            {
                byte TempAddr = Buf[i++];
                if ((TempAddr != ModBusMsg.SlaveAddr) & (TempAddr != BroadAddr)) { ModBusMsg.ErrorFlag = ModBus_AddrError; return 0; }
                if (Buf[i++] != ModBusMsg.MasterAddr) { ModBusMsg.ErrorFlag =ModBus_AddrError; return 0; }
            }
            ...
            if (ModBusMsg.MsgLength>0)
            {
               ...
               if (ModBusMsg.MsgLength != (ModBusMsg.DataLength + 3)) ModBusMsg.ErrorFlag = ModBus_SizeError;
            }
            if (ModBusMsg.CheakMode == CheakMode_Xor)
            {
                Cheak = Tools.Xor(Buf, i);
                if (Cheak != Buf[i]) ModBusMsg.ErrorFlag = ModBus_CheakError;
                else ModBusMsg.ErrorFlag = ModBus_Ok;i++;//xor校验一位
                 if (ModBusMsg.End != Buf[i++]) { ModBusMsg.ErrorFlag = ModBus_FlagError; return 0; }
            ...
            else if (ModBusMsg.CheakMode == CheakMode_Crc)
            {
                ...
                if (Cheak != Tools.ByteToInt16(Buf, i, 0)) ModBusMsg.ErrorFlag = ModBus_CheakError;
                else ModBusMsg.ErrorFlag = ModBus_Ok; i += 2;//crc校验2位
                if (ModBusMsg.End != Buf[i++]) { ModBusMsg.ErrorFlag = ModBus_FlagError; return 0; }
