...
{   MessageBox(NULL, L"Got unsupported interrupt", L"uCOS-II WIN32", MB_OK | MB_SETFOREGROUND | MB_ICONERROR);
}
...
{   char temp[256];
    ...
    {   if (WaitForSingleObject(hScheduleEvent, OS_SCHEDULER_TIMEOUT) == WAIT_TIMEOUT)	//Wait for a scheduler event (with timeout)
        {   sprintf(temp, "ERROR: Scheduler timed out in OSScheduleThread  %u --> %u   IF=%u  <-%u<%u<%u<%u<%u<%u<%u<%u<-\n",
            ...
            MessageBox(NULL, (LPCWSTR)temp, L"UCOS-II WIN32", MB_OK | MB_SETFOREGROUND | MB_ICONERROR);	//In case of timeout, display an error message ...
            OSRunning=0;
...
{   int i;
    ...
    {	sprintf(temp,"OSirq%u",i);
    	hInterruptEvent[i] = CreateEvent(NULL, TRUE, FALSE, (LPCWSTR)temp);
