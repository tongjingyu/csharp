...
BOOLEAN PC_GetKey(INT16S *c)
{
    ...
    {   MessageBox(NULL, L"Recursive call in PC_GetKey", L"ucos-II", MB_OK);
        exit(-1);
    } else if (lock)
    {	return FALSE;
    } else
...
{   COORD pos;
    ...
    if (PC_CHECK_RECURSIVE_CALLS && lock)			// Check and avoid recursive calls
    {   MessageBox(NULL, L"Recursive call in PC_DispStr", L"ucos-II", MB_OK);
        exit(-1);
    } else if (lock)
    {	return;
    } else
    ...
    if ((x>79) || (y>24))					// Check for valid cursor position
        MessageBox(NULL, L"Invalid screen position in PC_DispStr()", L"ucos-II", MB_OK);
...
{   COORD pos;
    ...
    if (PC_CHECK_RECURSIVE_CALLS && lock)			// Check and avoid recursive calls
    {   MessageBox(NULL, L"Recursive call in PC_DispChar", L"ucos-II", MB_OK);
        exit(-1);
    } else if (lock)
    {	return;
    } else
    ...
    if ((x>79) || (y>24))					// Check for valid cursor position
        MessageBox(NULL, L"Invalid screen position in PC_DispChar()", L"ucos-II", MB_OK);
...
{   COORD pos;
    ...
    if (PC_CHECK_RECURSIVE_CALLS && lock)			// Check and avoid recursive calls
    {   MessageBox(NULL, L"Recursive call in PC_DispClrScr", L"ucos-II", MB_OK);
        exit(-1);
    } else if (lock)
    {	return;
    } else
