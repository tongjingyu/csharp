...
namespace Geek1
{
    ...
    class Tools
    {
        ...
        public static String HexToString(byte[] str, int offet, int Length)
        {
            string String = "";
            try
            {
                for (int i = offet; i < (offet + Length); i++)
                {
                    String += str[i].ToString("X2");
                    String += " ";
                }
        ...
        public static byte[] StringToHex1(string s)
        {
        ...
        public static bool Comment(byte[] A, byte[] B)
        {
            int length;
            if (A.Length > B.Length) length = B.Length; else length = A.Length;
            if (length < 1) return false;
            for (int i = 0; i < length; i++)
            {
               if(i!=2)if (A[i] != B[i]) return false;
            }
            return true;
        }
    }
